...
export function activate(context: vscode.ExtensionContext) {
    ...
    const configChangeListener = vscode.workspace.onDidChangeConfiguration(
        (event) => {
            if (
                event.affectsConfiguration(extensionName) ||
    ...
    let disposable = vscode.commands.registerCommand(
        `${extensionName}.formatCurlyBracesOnNewLine`,
        () => {
...
async function autoCompleteOrNewLine(editor: vscode.TextEditor) {
    ...
    const didAutocomplete =
        cachedSettings.acceptSuggestionOnEnter &&
        (await acceptSelectedSuggestionAsync(editor));
...
async function formatCurlyBraces(editor: vscode.TextEditor) {
    ...
    const indentUnit = cachedSettings.insertSpaces
        ? " ".repeat(cachedSettings.tabSize)
        : "\t";
